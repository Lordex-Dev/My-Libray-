local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/Lordex-Dev/My-Libray-/main/LibOutono.lua"))()

local Window = Library:MakeWindow({
    Title = "Alpha Hub 👺",
    SubTitle = "By : Lordex",
    SaveFolder = "Alpha Hub 👺 Folder"
})

Window:AddMinimizeButton({
    Button = { Image = "rbxassetid://102541070817784", BackgroundTransparency = 0 }, -- Coloque um ID válido
...
Tab1:AddParagraph({"Olá, " .. playerName .. "!  Seja bem-vindo ao Alpha Hub 👺"})
Tab1:AddParagraph({"Versão Beta v0.01"})
Tab1:AddParagraph({"Atualizações em breve.."})

local Section = Tab1:AddSection("Créditos")

Tab1:AddParagraph({"Criador - Alpha_Lordex [Inscritos105] "})
Tab1:AddParagraph({"Administradora - Isabellasennay [Zzz] "})
Tab1:AddParagraph({"Administrador - Alpha_King [RENAN_01721] "})
...
local function FlingPlayer(player)
	...
	print("[Alpha] Ataque finalizado em:", player.Name, flingDetected and "Flingado" or "Sem Fling")
end
...
local Section = Tab2:AddSection({"Trolagens"})
...
local Section = Tab2:AddSection({"Fling Ball Beta"})
...
local Section = Tab2:AddSection({"Fling Couch"})
...
local Section = Tab2:AddSection({"Trolling cars"})
...
local Section = Tab2:AddSection({"Trolling All"})
...
Tab2:AddButton({
    Name = "Fast - Fling Couch All",
    Callback = function()
        local players = Players:GetPlayers() -- Pegue todos os jogadores
        local playerIndex = 1 -- Índice do jogador atual
        ...
        local function FlingNextPlayer()
            if playerIndex <= #players then
                ...
                -- Executa o código adicional (invocar o remoteFunction duas vezes)
                local args = {
                    [1] = "PickingTools",
                    [2] = "Couch"
                }
                ...
                if remoteFunction then
                    remoteFunction:InvokeServer(unpack(args)) -- Primeira execução
                    remoteFunction:InvokeServer(unpack(args)) -- Segunda execução
                end

                -- Loop para equipar o Couch duas vezes
                for i = 1, 2 do
                    EquipCouch() -- Equipa o Couch
                    task.wait(0.1) -- Pequeno atraso para garantir que o equip seja processado
                end

                -- Avança para o próximo jogador após executar o Fling e equipar o Couch
                playerIndex = playerIndex + 1
                FlingNextPlayer()
            end
        end
...
local Section = Tab3:AddSection({"Teleports"})
...
local Tab4 = Window:MakeTab({"Itens", "rbxassetid://10709769841"})

local Section = Tab4:AddSection({"Itens"})
...
local Section = Tab5:AddSection({"Funcionalidades"})
...
local Section = Tab5:AddSection({" Unban House"})
...
-- Atualiza opções do dropdown com pequeno atraso
local function UpdateDropdown()
    task.defer(function()
...
Tab9:AddSection({"Audio all [Custom]"})
...
Section = Tab9:AddSection({ Name = "Tornado de PaintRollers [Beta]" })
...
local Section = Tab10:AddSection({"Esp Playes"})
...
local Tab11 = Window:MakeTab({"Server Lag", "rbxassetid://10709806277"})

local Section = Tab11:AddSection({"Server Lag"})
